...
app.post("/CTC"), (req, res) => {
    ...
    const timeStartSala3 = ereq.body.timeStartSala3;
    ...
    ctc.query("SELECT * FROM ctc WHERE data = ?", [date], (err, result) => {
        if (err) {
            res.send(err);
            return;
        }
